...
Da SIP ein generisches Protokoll ist, ist es prinzipiell nicht nur für die Sprachkommunikation einsetzbar sondern auch um Daten zwischen Clients auszutauschen. Diese Kapitel soll zeigen, welche der bestehenden client-server und Peer-to-Peer Architekturen sich mit SIP umsetzen lassen. Es ist dabei nicht Ziel bestehende Probleme der einzelnen Architekturen lösen, indem das SIP Protokoll verwendet wird, sondern zu zeigen, dass SIP es ermöglicht bestehende Architekturen umzusetzen. Der resultierende Vorteil besteht vor allem in einer standardisierten Addressierung,  Signalisierung und hoher Abstraktionsebene.

In diesem Kapitel werden zunächst Client-Server Konzepte vorgestellt und dann eine die Implementierung einer einfache Client-Server Architektur auf Basis des SIP Protokolls vorgestellt. Diese soll zeigen, dass sehr einfach ein Spielprototyp gebaut werden kann, der SIP nicht nur für die Sprachkommunikation einsetzt sondern auch die Grundlage für eine Netzwerkschnittstelle bietet. 
...
\textbf{Persistenz:} Der Inhalt des Spieles kann Objekte die genau nur einmal existieren. Deswegen muss zwischen den einzelnen Sitzungen dafür gesorgt werden, dass diese persistent gespeichert werden.
...
\textbf{Reaktionsfreudigkeit:} Da Spiele oft die echte Welt simulieren, hat die Reaktionsfreudigkeit des Systems großen Einfluss darauf, für wie echt diese Illusion wahrgenommen wird. Werden durch zu große Verzögerungen Ereignisse zu spät dargestellt , so wird die Spielmechanik gestört. 
