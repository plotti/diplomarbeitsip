...
Im Spiel selbst ist der \textit{Irrlicht Scene Manager} für eine Verwaltung aller Objekte der Spielewelt verantwortlich. Er verfügt nicht nur über Koordinaten des Spielers und aller Objekte des Spiels, sondern auch über die die (X,Y,Z) Koordinaten aller anwesenden Spieler im Spiel.

Erhält der Spieler neue Positionsinformationen von anderen Teilnehmern, werden die Positionen der entsprechenden Spieler im SceneManager aktualisiert. Genauso versendet der Spieler selbst seine eigenen Koordinaten an alle Teilnehmenden Spieler.

Die lokale VoIP Instanz fragt periodisch (alle 10ms) beim SceneManager die Koordinaten aller aktiven Spieler ab, um die entsprechenden Distanzvektoren zu berechnen. Anhand dieser nimmt sie das Mischen der Audioströme der Spieler vor. Dies geschieht in der Hauptschleife von Irrlicht die hier auch alle Koordinaten der Spieler aktualisiert und das Bild neu zeichnet.

Unterschreitet ein Spieler die benötigte Entfernung zu einem anderen Spieler veranlasst die VoIP Instanz einen entsprechenden Anruf zu diesem Spieler. In Abhängigkeit von der Entfernung werden die Zonenkonzepte realisiert und durch die VoIP Instanz im Hintergrund eingeleitet.
...
Sowohl VoiP Anwendungen, als auch reguläre Spiele haben oft mit dem Problem zu kämpfen, dass einzelne Rechner hinter Routern und NATs nicht von außen erreichbar sind. Bei Spielen wurde das Problem bisher meistens manuell gelöst, indem Änderungen an Router, Firewall und NAT auf Clientseite vorgenommen worden sind, um entsprechende Ports freizugeben. 

Indem das STUN Protokoll eingesetzt wird, ist eine Teilnahme am Spiel von Spielern die Router einsetzen möglich, ohne dass sie Änderungen am Router vornehmen müssten. Die im Kapitel 4 vorgestellte Funktionsweise von STUN wird hier entsprechend direkt im Spiel angewandt. Dazu wird während der Registrierungsphase ein öffentlicher STUN kontaktiert, der es dem Spieler erlaubt seine öffentliche IP Adresse und den Typ des eingesetzten Routers herauszufinden. 
...
Auf dem Startbildschirm findet der Spieler seine bisherigen Freunde vor und kann ihren Status ablesen: Freunde können entweder offline, online oder gerade im Spiel sein. Möchte der Spieler das Spiel betreten, kann er einen aktiven Spieler anwählen und mit dem Drücken des \textit{Join}-Knopfs am Spiel teilnehmen. Möchte er ein neues Spiel starten, kann er mit dem Betätigen des \textit{Create}-Knopfes ein neues Spiel anlegen. 

\subsection{Neuer Spieler betritt das Spiel}
Betritt ein neuer Spieler das Spiel, so wird er von nun an vom \textit{Scene Manager} verwaltet und in der 3D Welt als neuer Avatar angezeigt. Dieser wird für jeden neuen Spieler an einer festgelegten Einstiegskoordinate erzeugt. Gleichzeitig wird die SIP Adresse des neuen Teilnehmers in die lokale \textit{Buddyliste} des Spielers aufgenommen. Befindet sich der entsprechende Spieler in Hörnähe wird ein Anruf zu diesem Teilnehmer in der VoIP Instanz ausgeführt.

\subsection{Spieler bewegt sich im Spiel}
Jeder Spieler ist in der Lage sich frei im Level zu bewegen. Jede Bewegung wird allen anderen Teilnehmern mitgeteilt, damit sie in der Lage sind ihre Spielewelt konsitent zu halten. Auf die gleiche Weise erhält der Spieler selbst erhält ebenfalls Bewegungsinformationen aller Spieler des aktiven Spiels. Diese Aktualisierung wird alle 10ms vorgenommen. Anhand der Positionsinformationen der Spieler wird die Sprachkommunikation entsprechend des Zonenkonzepts eingeleitet.
