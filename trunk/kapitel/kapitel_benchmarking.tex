...
Um den Bandbreitenverbrauch in den einzelnen Zonen aufzuzeigen, wurde ein einfaches Szenario erstellt, bei 2 Spieler am gleichen Ausgangspunkt das Spiel beginnen und sich dann voneinander wegbewegen. Während sie die einzelnen Zonen durchlaufen werden wird der Bandbreitenverbrauch eines Teilnehmers gemessen. Bei der Messung wird nicht nur zwischen dem Up- und Downloadverbrauch, sondern auch zwischen dem des SIP und RTP Protokolls unterschieden. Es wurde jeweils eine Messung für die drei Implementierungsvariationen (RTP-Silence-Supression, SIP-Hold, unterschiedliche Audiocodecs) vorgenommen. Diese sollen eine Reduzierung der benötigten Bandbreite bewirken.

\subsection{Ohne Zonen-Unterscheidung}
Hier entfernen sich beide Spieler voneinander, es werden jedoch keine bandbreitenreduzierenden Maßnahmen in Abhängigkeit von der Zone des Spielers getroffen. Man erkennt in der Abbildung \ref{fig:NOVADrtpsip}, dass die Bandbreite sich in allen Zonen gleichmäßig bei 10 kByte/s bleibt. 
...
\begin{figure}[tbh]
	\centering
		...
		\caption{Gegenüberstellung der SIP- und RTP-Bandbreite mit der Silence-Supression-Technik}
	\label{fig:VADRTPSIP}
\end{figure}
