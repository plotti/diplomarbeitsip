...
Obwohl der Audiocodec eine Bitrate von 16.8 Kbit/s besitzt, weist die tatsächliche verbrauchte Bandbreite jedes Rechners einen Upload von 5 KByte/s oder 40 Kbit/s auf, wie man in allen Messungenerkennen kann. Die Differenz zwischen der Nutzlast (Payload) und der tatsächlichen Bandbreite liegt im Header-Overhead. 
...
In der Abbildung \ref{fig:VADrtpsip} wurde ein Vergleich der SIP und RTP Protokolle dargestellt. Der Plot zeigt, dass bei der Silence Supression beim RTP Protokoll in der öffentlichen Zone die Bandbreite auf jeweils unter 1 KByte/s sinkt. Anhand der Unterscheidung der Protokolle wird deutlich, dass es sich um eine Eigenschaft des RTP-Protokolls handelt, da keine SIP Nachrichten beim Übergang erzeugt werden.
