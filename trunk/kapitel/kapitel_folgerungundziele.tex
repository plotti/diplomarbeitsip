...
Da der Hauptaspekt der Arbeit die Herstellung einer Sprachkommunikation besteht, sollte diese auch entsprechend des Kommmunikationsmodell von Shannon und Weaver \cite{shannon48} erfolgen. Wie in den vorgestellten Studien gezeigt wurde, basierten viele Probleme darauf, dass alle Spieler einen Kanals mit mehreren Spielern teilen müssen. Dies führte zu zwei Hauptproblemen:

1. Durch die Interferenzen von mehreren sich überlagerndedn Audioströmen nahm die Qualität des Mediums kontinuierlich ab, bis zu dem Punkt an dem viele Spieler die Sprachkommunikation als Störend empfanden.

2. Dadurch dass alle Spieler einen Kanal benutzten waren sie nicht in der Lage festzustellen an wen ihre Nachricht adressiert werden soll und ob die ankommenden Nachrichten an sie adressiert sind.

Dem Modell entsprechend haben wir bisher genau einen Kanal zur Übertragung mehrerer Audioinformationen genutzt, das dazu geführt hat, dass Störungen aufgetreten sind und keine eindeutige Adressierung mehr möglich war.
%Bild des Problems

Ersetzen wir einen Kanal durch mehrere Parallele Kanäle so können wir viele der genannten Probleme lösen: Wird nun zwischen jedem Spielerpaar nur ein Kanal aufgebaut, so können keine Störungen durch Interferenzen auftreten und jeder Spieler ist in der Lage genau zu bestimmen an wen seine Nachricht gehen soll. Auch der Feedback dieses Modells \cite{schramm54} kann so nun gezielt erfolgen. 
