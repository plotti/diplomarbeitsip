...
Dabei definieren wir folgende Komponenten:
...
\begin{itemize}
	...
	\item Konferenz Server: Ein Konferenz server ist ein physischer Server, der darüber entscheidet welche Audioströme miteinander gemischt werden und an wen sie verteilt werden. Dieser kann auch den Mixer beinhalten. 
	\item Teilnhemer: Ein software Element, das einen Benutzer mit einer Konferenz verbindet. Dieses implementiert mindestens einen SIP User Agent kann aber auch nicht SIP Spezifische Mechanismen für zusätzliche funktionalität beherbergen. 
	\item Spieleserver: Ein Spieleserver nimmt die Daten der Spieler entgegen, um sie auszuwerten und an die anderen Teilnehmer zu verteilen. Er unterstützt mindestens das SIP Protokoll. 
\end{itemize}
      
Bei Einer Umsetzung mit SIP stehen uns folgende vier Kombinationen zur Ver\-fügung: 
...
Der Hauptnachteil von Multicast Konferenzen ist jedoch dass jeder Router die Identitäten der Gruppe speichern muss. Außerdem kann jeder ohne Authentifizierung einer Multicast Gruppe beitreten. So verlieren die Teilnehmer die Kontrolle darüber wer an Konferenzen teilnehmen kann und sind auch nicht in der Lage sein zu wissen wer ihnen gerade zuhört. Ist bereits ein System falsch konfiguriert so könnte es allen existierenden Multicast Gruppen abonnieren und so das Netz fluten. Zusätzlich oder auch gerade auch wegen dieser Probleme wird Multicast in wenigen Internet Hauptleitungen unterstützt. So können Multicast Konferenzen zwar in LANs nützlich sein, sind jedoch im kommerziellen Internet nicht umsetzbar.
 ...
 In the loosely coupled model, there is no signaling
...
Sendet jeder Spieler seinen Audiostrom mit einer Speex \cite{speex08} 16 Kbit/s codierung an den Server, der nach dem heutigem Standard mit einer 100MBit Anbindung für den Upload und Download ausgestattet ist würde sich folgende Rechnung ergeben:
