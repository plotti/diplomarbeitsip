...
Obwohl die innewohnenden Probleme der vorgestellten Client-Server, Hybrid- und Peer-to-Peer-Architekturen in diesem Kapitel nicht gelöst werden soll ein Einblick gegeben werden, dass durch den Einsatz vom SIP-Protokoll eine standardisierte Adressierung,  Signalisierung und hohe Abstraktionsebene erreicht werden kann. Um die Umsetzung einer einfachen Client-Server Architektur mit SIP zu demonstrieren, wurde die Netzwerkschnittstelle beim Prototyp auf Basis des SIP realisiert und in diesem Kapitel beschrieben.
...
Prinzipiell sind in einem SIP-Standard alle Komponenten vorhanden, die man für eine P2P-Architektur braucht und übliche Probleme wie z.B. das NAT-Traversal bereits erfolgreich gelöst. Da die VoIP-Technologie ähnliche Konzepte und Probleme wie im Spielebereich aufweist, sind wichtige Komponenten bereits in Form von Lokationsdiensten, Präsenzdiensten, Proxys, Registraren oder STUN-Servern abstrahiert worden. P2P Ansätzen Spielebereich könnte diese Entwicklung noch bevorstehen. 
...
Alle Varianten haben gemeinsam, dass die einzige zentrale Komponente des Systems - der Registrar - entfernt wird und dezentral zwischen den einzelnen Teilnehmern verteilt mit DHT realisiert wird und jeder Knoten einen Teil der Funktionalität übernimmt. 
...
Bisherige Simulationen \cite{harjula06} von P2PSIP haben auch gezeigt, dass zwar durch die Signalisierung entstehende Latenz im Vergleich mit der klassischen serverbasierten SIP Variante beim REGISTER Vorgang ca. eine Sekunde länger dauert, jedoch der Nachrichtenversand über keine Latenzen verfügt, da er meist direkt stattfindet.
